...
export const S3Modal = ({ setModalIsOpen, s3ImageUrls, setImageFile, setSelectedS3Image }: Props) => {
  const [highlightedImage, setHighlightedImage] = useState("");
  const handleS3ImageApply = () => {
    setImageFile(null);
    setModalIsOpen(false);
    setSelectedS3Image(highlightedImage);
  };
  return (
      ...
      <div className="my-4">
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7 gap-3 overflow-auto max-h-96">
          {s3ImageUrls.map((url) => (
            <div
              key={url}
              className={`${highlightedImage === url && "border-8 border-blue-400"} w-24 h-24 sm:w-28 sm:h-28 relative`}
              onClick={() => {
